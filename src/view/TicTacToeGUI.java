...
public class TicTacToeGUI extends JFrame implements ActionListener {
    ...
    public TicTacToeGUI() {
        ...
        // bottom panel — reset button + score label
        ...
        scoreLabel = new JLabel(controller.getScoreboard().getScore(), JLabel.CENTER);
            ...
            for (int j = 0; j < 3; j++) {
                if (buttons[i][j] == clicked && buttons[i][j].getText().equals("")) {
                    ...
                    if (controller.checkWin()) {
                        statusLabel.setText(vsAI ? "You Win!" : controller.getCurrentPlayer().getName() + " Wins!");
                        statusLabel.setForeground(X_COLOR);
                        controller.getScoreboard().addPlayerWin();
                        scoreLabel.setText(controller.getScoreboard().getScore());
                        controller.setGameOver(true);
                        disableAllButtons();
                        return;
                    }
                    ...
                    if (controller.checkDraw()) {
                        statusLabel.setText("Draw!");
                        statusLabel.setForeground(Color.GRAY);
                        controller.getScoreboard().addDraw();
                        scoreLabel.setText(controller.getScoreboard().getScore());
                        controller.setGameOver(true);
                        disableAllButtons();
                        return;
                        ...
                        Timer timer = new Timer(700, ae -> {
                            ...
                            if (aiMove != null) {
                                ...
                                if (controller.checkWin()) {
                                    statusLabel.setText("You Lose!");
                                    statusLabel.setForeground(O_COLOR);
                                    controller.getScoreboard().addComputerWin();
                                    scoreLabel.setText(controller.getScoreboard().getScore());
                                    controller.setGameOver(true);
                                    return;
                                }
